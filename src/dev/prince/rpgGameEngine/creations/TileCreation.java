...
public class TileCreation extends Creation{
	...
	public void render() {
		...
		//RETURN TILE ID----------------------
		 if(GameState.prompt.getPromptText().toLowerCase().startsWith("/idof")&&Mouse.isButtonDown(1) ){
		...
		//REPLACING TILES FEATURE------------------------------
		 else if(GameState.prompt.getPromptText().toLowerCase().startsWith("/tiletotile") && Keyboard.isKeyDown(Keyboard.KEY_RETURN)){
			replaceTiles(Utils.parseInt(String.valueOf(GameState.prompt.getPromptText().charAt(12))),Utils.parseInt(String.valueOf(GameState.prompt.getPromptText().charAt(14))));
			}
