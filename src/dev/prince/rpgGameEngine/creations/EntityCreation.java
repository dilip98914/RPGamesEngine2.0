...
//	private int doorLength = 50;
	...
	public void render() {
		...
		mouseY = (int) (((handler.getHeight() - Mouse.getY()) + handler
				.getGameCamera().getyOffset()));
		// ADD ENTITIES
		// Move Entities
		boolean move = false;
		if (GameState.prompt.getPromptText().toLowerCase()
				.startsWith("/move entity")) {
			move = true;
		}
...
//			System.out.println(GameState.prompt.getPromptText().length());
		...
		if (GameState.prompt.getPromptText().equalsIgnoreCase("/USE NPC")) {// NPC
			...
			Renderer.renderSubImage(Assets.NPCsheet, mouseX
					- handler.getGameCamera().getxOffset(), mouseY
					- handler.getGameCamera().getyOffset(),
					Creature.DEFAULT_WIDTH, Creature.DEFAULT_HEIGHT,
					Assets.characterDown[0], 0.4f);
		}
		if (GameState.prompt.getPromptText().toLowerCase()
				.startsWith("/use staticentity")) {// STATIC ENTITY
			...
			if (GameState.prompt.getPromptText().length() > 18) {
				for (int i = 0; i < Assets.names.size(); i++) {
					if (GameState.prompt
							.getPromptText()
							.substring(18,
									GameState.prompt.getPromptText().length())
							.equalsIgnoreCase(Assets.names.get(i))) {// IF NAME
																		// MATCHES

						Renderer.setColor(1f, 1f, 1f, 0.5f);
						Renderer.renderSubImage(Assets.statics, mouseX
								- handler.getGameCamera().getxOffset(), mouseY
								- handler.getGameCamera().getyOffset(), sWidth,
								sHeight, Assets.staticEntities.get(i), 0.4f);
						makeStatic = true;
					}
				}
			}
			...
			if (Mouse.getEventButtonState()) {
				...
				if (makeDoor && Mouse.getEventButton() == 1) {
					...
					if (GameState.prompt.getPromptText().length() >= 11)
						doorData = GameState.prompt.getPromptText()
								.split("\\s+");
					if (doorData.length > 5) {
						Door d = ((doorData[5].equalsIgnoreCase("true")) ? new Door(
								handler, mouseX, mouseY,
								Utils.parseInt(doorData[1+2]),
								Utils.parseInt(doorData[2+1]),
								Utils.parseInt(doorData[0+2]), 20,
				...
				if (makeStatic && Mouse.getEventButton() == 1) {
					String tokens[] = GameState.prompt.getPromptText().split(
							"\\s+");
					handler.getWorld()
							.getEntityManager()
							.addEntity(
									new StaticEntity(handler, mouseX, mouseY,
											sWidth, sHeight,
					...
					if (move) {
						if (Mouse.getEventButton() == 1) {
							if (!(handler.getWorld().getEntityManager()
									.getEntity(mouseX, mouseY) == null)) {
								e = handler.getWorld().getEntityManager()
										.getEntity(mouseX, mouseY);
				...
				if (place) {
					...
					if (e.getClass().getSimpleName().equalsIgnoreCase("DOOR")) {
						Door d = (Door) e;
						// PREVIOUS DATA
						float tx = handler.getWorld().getEntityManager()
								.getPlayer().getX();
						float ty = handler.getWorld().getEntityManager()
								.getPlayer().getY();
						String oldPath = handler.getWorld().getWorldPath();

						if (handler.getWorld().getWorldPath()
								.equalsIgnoreCase(d.worlds[0])) {// BRING CHANGE
																	// IN
																	// CURRENT
																	// FILE
							d.x1 = mouseX;
						...
						if (handler.getWorld().getWorldPath()
								.equalsIgnoreCase(d.worlds[1])) {// BRING CHANGE
																	// IN
																	// CURRENT
																	// FILE
							d.x2 = mouseX;
						...
						for (int i = 0; i < d.worlds.length; i++) {// GET THE
																	// FILE TO
																	// DEAL WITH
							if (d.worlds[i].equalsIgnoreCase(handler.getWorld()
									.getWorldPath())) {
								handler.getWorld().setWorldPath(
										d.worlds[((i == 0) ? 1 : 0)]);
								handler.getWorld().loadWorld(
										d.worlds[((i == 0) ? 1 : 0)]);// new
																		// entity
																		// loaded
								break;
							}
						...
						for (Entity entity : handler.getWorld()
								.getEntityManager().getEntities()) {
							if ((entity.params.get(2).equalsIgnoreCase(
									e.params.get(2)) && entity.params.get(3)
									.equalsIgnoreCase(e.params.get(3)))
									|| (entity.params.get(0).equalsIgnoreCase(
											e.params.get(0)) && entity.params
											.get(1).equalsIgnoreCase(
													e.params.get(1)))
									&& (entity.params.get(4).equals(
											e.params.get(4))
											&& entity.params.get(5).equals(
													e.params.get(5))
											&& entity.params.get(7).equals(
													e.params.get(7)) && entity.params
											.get(6).equals(e.params.get(6)))) {
				...
				if (Mouse.getEventButton() == 1
						&& GameState.prompt.getPromptText().equalsIgnoreCase(
								"/DELETE entity")) {
					Entity e1 = (handler.getWorld().getEntityManager()
							.getEntity(mouseX, mouseY));
					if (e1 == null) {
						return;
					}
					if (e1.equals(handler.getWorld().getEntityManager()
							.getPlayer())) {
						return;
					}
					float tx = handler.getWorld().getEntityManager()
					...
					handler.getWorld().getEntityManager().getEntities()
							.remove(e1);// DOOR IN PRESENT FILE REMOVED
					...
					if (e1.getClass().getSimpleName().equalsIgnoreCase("Door")) {// REMOVE
						...
						for (Entity e11 : handler.getWorld().getEntityManager()
								.getEntities()) {
							if (e11.params.equals(e1.params)) {
								handler.getWorld().getEntityManager()
										.getEntities().remove(e11);
								break;
							}
						}
						// SAVE FILE
		...
		if (this.hold) {
			...
			if (e.getClass().getSimpleName().equalsIgnoreCase("StaticEntity")) {
				Entity e11 = handler.getWorld().getEntityManager()
						.getEntity(staticEntityX, staticEntityY);
				StaticEntity s = null;
